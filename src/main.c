...
#define APP_STORAGE_VERSION 1
Window *window;
Layer *clock_layer;
int clock_stroke = 1;
GColor clockColor;// = GColorWhite;
...
void handle_init(void) {
  ...
  //useSeconds = persist_read_bool(KEY_USE_SECONDS);
  clock_stroke = persist_read_int(KEY_THICKNESS);
  
    APP_LOG(APP_LOG_LEVEL_INFO, "Config: color: %d, bg: %d, stroke: %d",color, background, clock_stroke);
